...
static GLint staffLineArray[STAFFLINEDATASIZE];
static GLint staffQuadArray[STAFFQUADDATASIZE];

// -- this to be positioned using transform calls --
static GLint noteDataArray[NOTEDATASIZE];
...
static void drawMusicSymbols()
{
    ...
    glInterleavedArrays(GL_T2F_N3F_V3F,0,Treble_vertex);
    glDrawElements(GL_TRIANGLES,Treble_polygoncount*3,GL_UNSIGNED_INT,Treble_index);
    ...
    glInterleavedArrays(GL_T2F_N3F_V3F,0,Bass_vertex);
    glDrawElements(GL_TRIANGLES,Bass_polygoncount*3,GL_UNSIGNED_INT,Bass_index);
...
static void drawStaffDataQuads(float x, float y)
{
    ...
    glTranslatef(x, y, 0);    
    glEnableClientState(GL_VERTEX_ARRAY);    
    glVertexPointer(VERTEX_SIZE, GL_INT, 0, staffQuadArray);
    glDrawArrays(GL_QUADS, 0, STAFFQUADVERTEXCOUNT);
...
static void drawEllipse(int x, int y, float scale)
{
    ...
    glEnableClientState(GL_VERTEX_ARRAY);    
    glVertexPointer(2, GL_INT, 0, noteDataArray);
...
static void drawSharp(int x, int y, float scale)
{
    ...
    glInterleavedArrays(GL_T2F_N3F_V3F,0,Sharp_vertex);
    glDrawElements(GL_TRIANGLES,Sharp_polygoncount*3,GL_UNSIGNED_INT,Sharp_index);
